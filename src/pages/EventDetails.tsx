...
import { Calendar, MapPin, Users, Clock, ExternalLink, Star, Heart, MessageCircle } from "lucide-react";
...
const EventDetails = () => {
  ...
  return <div className="min-h-screen" style={pageStyle.background ? {
      ...
      <main className="container mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-16">
          ...
          <div className="lg:col-span-2">
            ...
            {/* Tags */}
            {event.tags && <div className="flex flex-wrap gap-2 mt-8">
                {event.tags.map((tag: string) => <Badge key={tag} variant="outline" className="border-primary/30 text-primary bg-primary/10">
                    {tag}
                  </Badge>)}
              </div>}
          </div>
              ...
              <CardContent className="p-6 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div>
                    ...
                    {event.city && <p className="text-black text-sm">{event.city}</p>}
                    {event.mapLink && <a href={event.mapLink} target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 text-sm inline-flex items-center gap-1 mt-1">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-2xl font-bold text-primary">
                    {event.isFree !== undefined ? event.isFree ? "Gratuito" : `R$ ${event.ticketPrice},00` : event.price || "Gratuito"}
                  </p>
                </div>

                {/* Capacity */}
                {(event.capacity || event.attendees) && <div className="space-y-2">
                    <div className="flex items-center gap-2 text-primary">
                      <Users className="h-5 w-5" />
                      <p className="text-sm text-black">Vagas</p>
                    </div>
                    <p className="text-black font-medium">
                      {event.attendees ? `${event.attendees} / ${event.capacity} participantes` : event.capacity ? `Até ${event.capacity} participantes` : 'Capacidade não definida'}
                    </p>
                  </div>}

                {/* Organizer */}
                {event.organizer && <div className="space-y-2">
                    <p className="text-sm text-black">Organizador</p>
                    <div className="flex items-center gap-3">
                      <img src={event.organizerLogo} alt={`${event.organizer} logo`} className="w-10 h-10 rounded-full object-cover" />
        ...
        {(event.isUserRegistered || event.matchmakingEnabled) && <section className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
              {mockMatches.map(match => <Card key={match.id} className="card-bridge-interactive group cursor-pointer">
                  <CardContent className="p-4 text-center">
                    <div className="relative mb-4">
                      <img src={match.photo} alt={match.name} className="w-20 h-20 rounded-full mx-auto object-cover" />
